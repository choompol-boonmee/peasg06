...
impl ReportTemp {
    ...
    async fn new(wk5prc: Arc<RwLock<wk5::Wk5Proc>>, ssid: String) -> Self {
        ...
        let title = "IRR CALCULATION FOR PROVINCE ".to_string();
		
		let mut repo = Report::default();
		...
		for s in 0..wk.ssv.len() {
			//if wk5prc.ssv[s].ssid!="BLA" { continue; }
			//if wk.ssv[s].ssid!=ssid { continue; }
			if wk.ssv[s].prov!=ssid { continue; }
			s_list.push(s);
		}
		...
		for s in s_list {
			for f in 0..wk.ssv[s].feeders.len() {
				let fd = &wk.ssv[s].feeders[f];
		...
		let mut cashflow = vec![];
		for cf in &rw_cash_flow.amts {
			let cf = *cf as f64;
			cashflow.push(cf);
		}
		let irr = financial::irr(&cashflow, None).unwrap();
		rw_cash_flow.sum1 = irr as f32 * 100.0;
...
impl Report {
    pub fn dava(&self, r: usize, c: usize) -> dcl::DaVa {
		let rw = &self.rows[r];
        //let ss = &ssv[s];
...
pub async fn handler(Path(ssid): Path<String>) -> ReportTemp {
    ReportTemp::new(base().wk5prc.clone(), ssid.to_string()).await
}
