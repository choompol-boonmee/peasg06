...
use calamine::{deserialize_as_f64_or_none, open_workbook, RangeDeserializerBuilder, Reader as XlsReader, Xlsx, Data};
...
pub async fn pea_sub_xml_read() -> Result<(), Box<dyn std::error::Error>> {
    let fd = format!("{}\\ผัง single line diagram สถานีไฟฟ้า\\CIM_PEA_Substation\\Exports",src_dir());
    println!("{}", fd);
    let mut cn = 0;
    if let Ok(paths) = read_dir(fd) { // if paths
        let mut subxv = vec![];
        for pt in paths { // for pt
            if let Ok(pt) = pt { // if pt
                ...
                if let Ok(mut xrd) = XmlReader::from_file(xmlf) {
                    ...
                    //println!("xml OK {}", cn);
                    loop { // xml
                        match xrd.read_event_into(&mut xbuf) { // xml match
                            Ok(Event::Eof) => break,
                            Ok(Event::Start(e)) => { // e
                                txts = "".to_string();
                                lv += 1;
        ...
        if let Ok(ser) = bincode::serialize(&subxv) { // if serialize
...
pub async fn pea_spp_lp_read() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for yr in yrs {
        ...
        if let Ok(mut rdr) = csv::Reader::from_path(&lp) { // if read file
            ...
            if let Ok(ser) = bincode::serialize(&lpv) { // if serialize
...
pub async fn pea_sub_excel() -> Result<(), Box<dyn std::error::Error>> {
    let file = format!("{}/ข้อมูลสถานีไฟฟ้าทุกเขต/GOC Substation & Another Detail.xlsx",src_dir());
...
pub async fn pea_vspp_lp_read() -> Result<(), Box<dyn std::error::Error>> {
    let yrs = vec!["2022","2023"];
    for yr in yrs {
        let lp = format!("{}\\ข้อมูล P, Q VSPP\\VSPP {}\\VSPP {}.csv",src_dir(), yr, yr);
        println!("{}", lp);
        if let Ok(mut rdr) = csv::Reader::from_path(&lp) { // if read file
...
pub async fn pea_vspp_excel() -> Result<(), Box<dyn std::error::Error>> {
    let file = format!("{}/ข้อมูลSPP, VSPP ที่เชื่อมต่อ กฟภ/สถานะ VSPP + SPP (กรอ.) - updated.xlsx",src_dir());
    println!("FILE {}", file);
    let mut excel: Xlsx<_> = open_workbook(file)?;
    ...
    if let Ok(range) = excel.worksheet_range("SPP") {
        for row in range.rows() {
            ...
            let sppinf = SPPInfo {ord,code,tamb,amp,prov,abbr,fuel,area,kv,conn};
            spp_info.push(sppinf);
        }
        if let Ok(ser) = bincode::serialize(&spp_info) { // if serialize
    ...
    if let Ok(range) = excel.worksheet_range("VSPP") {
        ...
        for row in range.rows() {
            ...
            let vsppinf = VSPPInfo {ord,code,area,tamb,amp,prov,fuel,kv,conn,fdno,circ,ppid};
            vspp_info.push(vsppinf);
        }
        if let Ok(ser) = bincode::serialize(&vspp_info) { // if serialize
...
pub async fn pea_lv_solar_xlsx() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(dr) = vdir.pop() {
        if let Ok(paths) = read_dir(dr) { // if paths
            for pt in paths { // for pt
                if let Ok(pt) = pt { // if pt
                    let pt = pt.path();
    ...
    if let Ok(xlsv) = xlsx_info(&flst).await {
        ...
        if let Ok(ser) = bincode::serialize(&xlsv) { // if serialize
...
pub async fn pea_sub_plan() -> Result<(), Box<dyn std::error::Error>> {
    let flst = vec![format!("{}\\ข้อมูลแผนงานก่อสร้างสถานีไฟฟ้า\\Report Substation Load Forecast - Report ม.ค. 2566.xlsx",src_dir())];
    println!("{:?}", flst);
...
pub async fn pea_load_fore() -> Result<(), Box<dyn std::error::Error>> {
    let flst = vec![format!("{}\\ข้อมูลพยากรณ์โหลด\\Report Detail Substation Load Forecast_กวร. ม.ค. 66.xlsx",src_dir())];
    println!("{:?}", flst);
    if let Ok(xlsv) = xlsx_info(&flst).await {
        println!("xlsv: {}", xlsv.len());
        for x in &xlsv {
...
pub async fn pea_meter_read() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for fl in flst {
        ...
        let fl = format!("{}/20240801_กรอ/export_กรอ_bil013_{}.csv", src_dir(), fl);
        println!("start {}", &fl);
        if let Ok(mut rdr) = csv::Reader::from_path(&fl) { // if read file
            ...
            let csv = CSVFile { path, name, rcnt, ccnt, data,};
            csv_v.push(csv);
            print!("read '{}' r:{} c:{}\n", fl, rcnt, ccnt);
        }
    }
    println!("start saving");
    if let Ok(ser) = bincode::serialize(&csv_v) { // if serialize
...
pub async fn pea_sub_do() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for rw in &repl.data {
        ...
        let id = if rw[5].len()>0 { rw[5][0..3].to_string() } else { "?".to_string() };
        let sb = if rw[6].len()>0 { rw[6].to_string() } else { "?".to_string() };
        let sz = rw[7].to_string();
        let nm = if let Some(sbif) = subxls.get(&id) { sbif.name.to_string() } else { "?".to_string() };
        let th = if let Some(thid) = thnm.get(&sb) { thid.to_string() } else { "?".to_string() };
        ...
        let sbid = if tp=="1" { th.to_string() } else { fd.to_string() };

        if sbid.len()==3 {
            println!("Y:{} ID:'{}' NM:'{}' TH:'{}' IX:'{}' = {} - {}.", yr, id, sb, nm, th, tp, fd);
            let repln = REPlan { year, apid, proj, feed, sub, pwmw, cate, sbid };
            newre.push(repln);
        }
    }
    if let Ok(ser) = bincode::serialize(&newre) { // if serialize
                ...
                for r in range.rows() {
                    ...
                    for (i, c) in r.iter().enumerate() {
                        let s = match c {
                            ...
                            Data::String(ref s) | Data::DateTimeIso(ref s) | Data::DurationIso(ref s) => s.to_string(),
                            v => v.to_string(),
                        };
                        rr.push(s);
                    }
    ...
    while let Some(be) = bess.pop() {
        for rw in &be.data {
            if let Some(s) = ennm.get(&rw[1]) {
                ...
                let stor = StoragePlan { sbid, sbnm, pwmw, emwh, year, yrno };
                println!("sto: {:?}", stor);
                newsto.push(stor);
            }
        }
