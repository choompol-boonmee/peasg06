...
impl ReportTemp {
    ...
    async fn new(wk5prc: Arc<RwLock<wk5::Wk5Proc>>) -> Self {
        ...
        let title = "SOLAR BATT CALCULATION : WK5E".to_string();
...
pub async fn make_repo(wk5prc: &mut wk5::Wk5Proc, acfg: Arc<RwLock<dcl::Config>>) {
    ...
    for s in 0..wk5prc.ssv.len() {
        for f in 0..wk5prc.ssv[s].feeders.len() {
            ...
            rw.solar_year_price = rw.solar_year_store * esp;
            rw.solar_all_price = rw.solar_year_price * yea;
            rw.solar_sum_price = rw.solar_all_price;
            rw.bess_cost = rw.solar_store * opy * 365.0 * unp;
            rw.solar_net_rev = rw.solar_all_price - rw.bess_cost;
...
impl Report {
    pub fn dava(&self, ssv: &Vec<wk5::Substation>, r: usize, c: usize) -> dcl::DaVa {
        ...
        match c {
            ...
            6 => DaVa::F32(self.rows[r].solar_store),
            7 => DaVa::F32(self.rows[r].bess_cost),
            8 => DaVa::F32(self.rows[r].solar_all_price),
            9 => DaVa::F32(self.rows[r].solar_net_rev),
            10 => DaVa::F32(self.rows[r].solar_sum_price),
            n => DaVa::USZ(n),
        }
    }
}
